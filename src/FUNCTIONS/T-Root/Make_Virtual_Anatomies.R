...
Make_Virtual_Anatomies <- function(Virtual_Roots,
                                   MECHA_path,
                                   output_path,
                                   anatomy_path, 
                                   done_path,
  ...
  # estimated_time <- length(unique(Virtual_Roots$root)) * length(unique(Virtual_Roots$x)) * 0.5
  ...
  template <- read_param_xml(template_path)    # for adding the rest of the data to the params
    ...
    for(temp_x in unique(Virtual_Roots$x)){
      ...
      tryCatch(
        expr= {
                ...
                print("     Success of GRANAR execution. Initializing MECHA...")
                ...
                py_run_file("MECHA/MECHAv4_TRoot.py")
                ...
                Temp_K <- data.frame(Root = temp_root,
                                     ...
                                     Kx = c(Kx_0, Kx_1, Kx_2, Kx_3, Kx_4, Kx_5, Kx_6, Kx_7, Kx_8, Kr_9),
                                     perimeter = peri)
                
                write.csv(Temp_K, file = paste0(result_path, temp_root,"_",temp_x,".csv"), row.names = F)
      ...
      # Write that the iteration has been done
