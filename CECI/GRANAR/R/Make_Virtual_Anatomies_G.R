...
Make_Virtual_Anatomies_G <- function(Virtual_Roots)
  ...
  estimated_time <- length(unique(Virtual_Roots$root)) * length(unique(Virtual_Roots$x)) * 6
  print(paste0("Estimated time : ", estimated_time, " sec."))
  t0 <- Sys.time()
    ...
    for(temp_x in unique(Virtual_Roots$x)){
      ...
      tryCatch(
        expr= {
                ...
                write_anatomy_xml(sim1, paste0("Data/Current_roots/CR_",temp_root,"_", temp_x,".xml"))
