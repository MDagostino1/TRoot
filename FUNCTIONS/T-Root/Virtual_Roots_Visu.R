...
vroot_plot2 <- function(Data = data,
  ...
  ggplot(data = Data,
         aes(x = x,y = y)) +                                                   # Change here to add/supp colored root
    geom_point(size = 1, color = "red3", alpha = 0.2) +
...
Virtual_Roots_Visu2 <- function(Virtual_Roots){
  ...
  # Merge plots
  p <- plot_grid(plot2, plot3, plot4, plot5, plot6, plot7, plot8, plot12, plot14, plot16, plot17, plot18, plot19, plot20, plot21,
                 ncol = 3,
                 theme_bw(),
                 align = 'v', axis = 'lr')
