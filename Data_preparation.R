...
Parameters <- read.csv(paste0(data_path, "Parameters.csv"))[-1]

print("Creating anatomy1...")
anatomy1 <- Merge_Quantifications("Data/Quantifications/")
...
source("GRANAR/R/T-Root/Functions.R")
...
for(i in seq(1:length(list))){
  ...
  data_all2 <- merge(data_all, Parameters[,c(1,2,3)])
  # print(data_all2$value[data_all2$name == "phloem" & data_all2$type == "n_layers"])

  data_all <- data_all2[c(4,3)]
  merged1 <- merge(temp_df, data_all) # Merge ids and data
  anatomy2 <- rbind(anatomy2, merged1)
}
...
anatomy2a <- merge(x = anatomy2, y = Parameters[,c(1,2,3)], by = "param_id")
...
# Anatomy1
anatomy1 <- merge(x = anatomy1, y = CrossSections)
anatomy1 <- anatomy1[c("CS_id", "name", "type", "value")]
anatomy1$id <- seq.int(nrow(anatomy1))

# Anatomy2
anatomy2 <- merge(x = anatomy2, y = CrossSections)
