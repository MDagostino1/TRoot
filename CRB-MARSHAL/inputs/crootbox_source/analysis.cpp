...
void SegmentAnalyser::write(std::string name)
{
	...
	if (ext.compare("vtp")==0) {
		...
		std::cout << "writing text file for TXT import: "<< name << "\n";
		writeRBSegments(fos);
...
/*void SegmentAnalyser::writeRBSegments(std::ostream & os) const
{
	...
	int nid1 = 0;
	int nid2 = 1;
	int branchnumberprev = 0; // first branch id
	...
	for (size_t i=0; i<segments.size(); i++) {
		...
		if(branchnumber != branchnumberprev){ // This means we are in a new branch and we need to import the connection

			branchnumberprev = branchnumber;
			Root* parentroot = r->parent;
			int parentnodeid  = r->parent_ni;
			Vector2i s = segments.at(i);
			Vector3d n1 = nodes[parentnodeid];
			Vector3d n2 = nodes.at(s.y);
			double radius = r->param.a;
			double length = sqrt((n1.x - n2.x)*(n1.x - n2.x) + (n1.y - n2.y)*(n1.y - n2.y) + (n1.z - n2.z)*(n1.z - n2.z));
			double red = r->getRootTypeParameter()->colorR;
			...
			double time = ctimes.at(i);
			double type = r->param.type;
			os << std::fixed << std::setprecision(4)<< parentnodeid << " "  << nid2 << " " << branchnumber << " " << n1.x << " "  << n1.y << " " << n1.z << " " << n2.x << " " << n2.y << " " << n2.z << " " <<
					radius << " " << length << " " << red << " " << green << " " << blue << " " << time<< " " << type << " \n";
